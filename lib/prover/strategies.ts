...
export const STRONGHOLD_STRATEGIES: StrategyDef[] = [
  ...
  },
  {
    ...
    note: "Before the planner sees the theorem, the bridge runs a breadth-first reconnaissance sweep on the ghost army: it opens the goal as a live state, asks `apply?` (via Leak IV) which Mathlib theorems unify with it, races those plus a standard tactic portfolio against the state with branch_tactics, and expands EVERY state that advanced — mapping the neighbourhood rather than tunnelling down one line (default \u22643 levels, \u226424 states, \u22644 min). The planner is then briefed with what actually moved each goal, which theorems `apply?` named (their hypotheses are a decomposition that composes by construction), and which states resisted everything \u2014 the last being where the proof is genuinely hard. The briefing is framed as evidence, not a proposal, and the planner is told to decide fast, because the mechanical exploration is already done and its scarce resource is judgement. Every hole gets its own smaller sweep before its minion launches \u2014 including sub-holes created by a mid-run split, so the evidence does not decay with depth. A hole's goal is lifted to a closed proposition first (\u2200 over the theorem's binders plus every earlier `have` as a hypothesis), which over-abstracts deliberately: anything that closes the probe certainly closes the real hole. All recon shares one run-wide budget (default \u226425% of the clock) and is cached by goal shape, so recursion can never starve the provers. Downstream is Surround unchanged, with the progress gate on. Fail-soft: no Pantograph, an unreadable proposition, or a Leak IV without the suggestion patch each degrade it toward plain Surround. Requires the ghost-army Leak II; `apply?` harvesting additionally needs the Leak IV severity-3 suggestion patch deployed.",
  },
];
