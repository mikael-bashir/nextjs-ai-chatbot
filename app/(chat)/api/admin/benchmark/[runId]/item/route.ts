...
// PATCH one claimed item. Three shapes, distinguished by `action`:
//   { action: 'outcome', itemId, status, proof?, costUsd?, ... }  — a completed
//     attempt (proved / refuted / unsolved) — always a genuine result.
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (body.action === 'outcome') {
      if (!['proved', 'refuted', 'unsolved'].includes(body.status)) {
